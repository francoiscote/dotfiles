...
export default function (pi) {
  ...
  function onBlocked(data: { active?: boolean; label?: string } | undefined) {
    if (!rootSession) {
      return;
    }
    if (!data?.active) {
  ...
  // rpiv-ask-user-question emits this when ask_user_question tool blocks waiting for input
  pi.events.on("rpiv:ask-user:blocked", (data) => {
    onBlocked({ active: data?.active });
  });
